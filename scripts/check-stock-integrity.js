...
async function main() {
  const branchVsLocation = await prisma.$queryRaw`
    SELECT p.name AS product, b.name AS branch, ps.stock AS branch_stock,
           COALESCE(loc.total, 0)::int AS location_stock
  ...
  const companyVsBranch = await prisma.$queryRaw`
    SELECT p.name AS product, p.stock AS company_stock,
           COALESCE(SUM(ps.stock), 0)::int AS branches_stock
    FROM products p
  ...
  for (const row of branchVsLocation) {
    console.log(`sucursal ≠ ubicaciones: ${row.product} @ ${row.branch}: ${row.branch_stock} vs ${row.location_stock}`)
  }
  for (const row of companyVsBranch) {
    console.log(`empresa ≠ sucursales: ${row.product}: ${row.company_stock} vs ${row.branches_stock}`)
  }
